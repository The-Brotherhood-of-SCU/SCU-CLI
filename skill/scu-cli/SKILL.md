...
scu user balance    # 校园卡余额（取最近一笔交易余额；数据源：电子卡交易记录 cardhis）
scu user transactions [--sdate YYYY-MM-DD] [--edate YYYY-MM-DD]   # 校园卡扣费/充值明细（日期可省略，省略时服务端返回最近几天；输出 transactions 平铺明细 + by_day 每日消费合计 + total_cost/balance）
```
