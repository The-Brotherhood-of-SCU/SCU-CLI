...
| `balance` / `transactions [--sdate …] [--edate …]` | 校园卡余额 / 扣费充值明细（数据源：电子卡交易记录 cardhis；日期可省略，输出每日合计与总消费） |
...
| `add --mac <MAC> [--days N] [--exit 运营商]` | 绑定设备 MAC（12 位十六进制无分隔符，冒号/连字符写法自动归一化；绑定后连校园网自动认证；`--days 0` 为最长 6 年） |
